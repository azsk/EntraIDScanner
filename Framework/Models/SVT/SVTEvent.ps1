...
class ControlResult
{
	...
	[string] $DetailedResult = [Constants]::DefaultAdditionalDetail;
...
class SVTEventContext: AzSKRootEventArgument
{
	...
	[string] GetUniqueId()
	{
		...
		{
			$uniqueId = $this.TenantContext.Scope;
		}

		# Unique Id validation
		if([string]::IsNullOrWhiteSpace($uniqueId))
		{
			throw "Error while evaluating Unique Id. The parameter 'ResourceContext.ResourceId' OR 'TenantContext.Scope' is null or empty."
		}
