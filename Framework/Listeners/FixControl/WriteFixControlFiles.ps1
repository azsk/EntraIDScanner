...
Repair-AzSKSubscriptionSecurity `
	...
	hidden [void] InitializeFolder([TenantContext] $subContext, [string[]] $fixControlFileNames, [string] $runScriptContent)
	{
		...
		$fixControlFileNames | ForEach-Object {
			New-Item -ItemType Directory -Path ([System.IO.Path]::Combine($($this.FolderPath), "Services")) | Out-Null
			Copy-Item ($parentFolderPath + $_) ([System.IO.Path]::Combine($($this.FolderPath), "Services", $($_)))
		};
	}
	...
	[void] CommandCompletedAction([SVTEventContext[]] $arguments)
	{
		if($arguments -and $arguments.Count -ne 0)
		{
			...
			if($fixControlEventContext.Count -ne 0)
			{
				...
				$fixControlEventContext | Group-Object { $_.TenantContext.tenantId } | 
				ForEach-Object {
					$sub = $_.Group;
					$subObject = [FixControlConfig]@{
						TenantContext = $sub[0].TenantContext;
					};
					$output += $subObject;
				...
				if($output.Count -ne 0)
				{
					...
					$this.InitializeFolder(($fixControlEventContext | Select-Object -First 1).TenantContext, $fixControlFileNames, $runScriptContent);
