...
{   
    ...
    [void] RegisterEvents()
    {
        ...
        $this.RegisterEvent([SVTEvent]::CommandStarted, {
            ...
            try 
            {
                $currentInstance.SetFilePath($Event.SourceArgs.TenantContext, ("SecurityReport-" + $currentInstance.RunIdentifier + ".csv"));
            }
            catch 
            {
                $currentInstance.PublishException($_);
        ...
        $this.RegisterEvent([SVTEvent]::CommandCompleted, {
			...
			{
				$currentInstance.SetFilePath($Event.SourceArgs[0].TenantContext, ("SecurityReport-" + $currentInstance.RunIdentifier + ".csv"));
			}
			else
			{
				...
				$currentInstance.SetFilePath($Event.SourceArgs[0].TenantContext, ("AttestationReport-" + $currentInstance.RunIdentifier + ".csv"));
			}
        ...
        $this.RegisterEvent([AzSKRootEvent]::UnsupportedResources, {
            ...
            try 
            {
				...
				{
					$filePath = $currentInstance.CalculateFilePath($Event.SourceArgs.TenantContext, [FileOutputBase]::ETCFolderPath, ("UnsupportedResources-" + $currentInstance.RunIdentifier + ".csv.LOG"));
		...
		$this.RegisterEvent([AzSKRootEvent]::WriteCSV, {
            ...
            try 
            {
				...
				if($message -and $message.DataObject)
				{
					...
					$filePath = $currentInstance.CalculateFilePath($Event.SourceArgs.TenantContext, $folderPath, ($fileName + "." + $fileExtension));
		...
		$this.RegisterEvent([AzSKRootEvent]::WriteExcludedResources,{
            ...
            try 
            {
				...
				if($message -and $message.DataObject)
				{
					$filePath = $currentInstance.CalculateFilePath($Event.SourceArgs.TenantContext, [FileOutputBase]::ETCFolderPath, ("ExcludedResources-" + $currentInstance.RunIdentifier + ".txt.LOG"));
        ...
        $arguments | ForEach-Object {
            ...
            if ($item -and $item.ControlResults) {
                $item.ControlResults | ForEach-Object{
					...
					{
					    $csvItem.ResourceId = $item.TenantContext.scope;
						$csvItem.DetailedLogFile = "/$([Helpers]::SanitizeFolderName($item.TenantContext.TenantName))/$($item.FeatureName).LOG"
					}
