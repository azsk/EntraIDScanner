...
class CommandBase: AzSKRoot {
    ...
    CommandBase([string] $tenantId, [InvocationInfo] $invocationContext):
    Base($tenantId) {
        ...
        try {
            if (-not [string]::IsNullOrEmpty($folderpath)) {
                switch ($GeneratePDFReport) {
                    ...
                    Landscape {
                        [AzSKPDFExtension]::GeneratePDF($folderpath, $this.TenantContext, $this.InvocationContext, $true);
                    }
                    Portrait {
                        [AzSKPDFExtension]::GeneratePDF($folderpath, $this.TenantContext, $this.InvocationContext, $false);
                    }
                }
            }
        }
