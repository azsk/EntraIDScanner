...
class SVTBase: AzSKRoot
{
    ...
    SVTBase([string] $tenantId, [SVTResource] $svtResource):
        Base($tenantId)
    {		
		$this.CreateInstance($svtResource);
    }

	SVTBase([string] $tenantId):
        Base($tenantId)
    {
		$this.CreateInstance();
    }

	SVTBase([string] $tenantId, [string] $resourceGroupName, [string] $resourceName):
        Base($tenantId)
    {
		$this.CreateInstance([SVTResource]@{
	...
	hidden [string] GetResourceId()
    {
		if ([string]::IsNullOrEmpty($this.ResourceId))
		{
			...
			{
				$this.ResourceId = $this.TenantContext.Scope;
			}
		}
	...
	hidden [void] PostProcessData([SVTEventContext] $eventContext)
	{
		...
		try
		{
			...
			if([FeatureFlightingManager]::GetFeatureStatus("EnableAzurePolicyBasedScan",$($this.TenantContext.tenantId)) -eq $true)
			{
