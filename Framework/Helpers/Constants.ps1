...
"        c) If the control status says 'Skipped', it means that you have chosen to skip certain controls using the '-SkipControlsFromFile' parameter.`r`n" 
	...
	static [string] $DefaultInfoCmdMsg = "This command provides overall information about different components of the AzSK which includes subscription information, security controls information, attestation information, host information. 'Get-AzSKInfo' command can be used with 'InfoType' parameter to fetch information.`r`n" + 
					...
					"`r`n`r`nExamples:`r`n" +
					"`tGet-AzSKInfo -InfoType SubscriptionInfo -tenantId <YourtenantId> `r`n" +
					"`tGet-AzSKInfo -InfoType ControlInfo -ResourceTypeName All -UseBaselineControls `r`n" +
					"`tGet-AzSKInfo -InfoType AttestationInfo -tenantId <YourtenantId> -ResourceTypeName All -UseBaselineControls `r`n" +
    ...
    "`r`nStarting analysis: [FeatureName: {0}] [TenantName: {1}] [tenantId: {2}] `r`n" + [Constants]::SingleDashLine
    ...
    static [string] $CompletedAnalysisSub = [Constants]::SingleDashLine + "`r`nCompleted analysis: [FeatureName: {0}] [TenantName: {1}] [tenantId: {2}] `r`n" + [Constants]::DoubleDashLine
    ...
    "`r`nInfo: Starting attestation - [FeatureName: {0}] [TenantName: {1}] [tenantId: {2}] `r`n" + [Constants]::SingleDashLine
	...
	static [string] $BlanktenantId = "00000000-0000-0000-0000-000000000000"
	...
	static [string] $DetailedResults = "No additional details found. Please check the detailed logs."
